...
words <- c("drum","slipper","diver","funeral","lift","navel")
...
words_of_the_day <- paste(sQuote(words), "is the word of the day!")
...
word_bin <- function(vec_of_words, letter_1, letter_2){
  out_words <- vec_of_words[(vec_of_words >= letter_1) & (vec_of_words < letter_2)]
  return(out_words)
}
